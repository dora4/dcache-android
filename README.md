...
API	Description
checkService	Checks if an API service is available. If unavailable, mappingBaseUrl() was not called during initialization to map the Base URL.
getService	Retrieves an API service object.
removeService	Removes an API service object.
mappingBaseUrl	Binds a Base URL to an API service.
Getting Started
kotlin
复制代码
...
Integrating Other Mainstream ORM Frameworks
